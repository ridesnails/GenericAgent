...
- 建渠道 `CreateChannelInput`：必填 `type! name! credentials{apiKey...} supportedModels! defaultTestModel!`（+可选 baseURL/status/…）。
...
# 启用/禁用渠道（先 channels 拿到 id）
python axonhub.py graphql 'mutation($id:ID!,$i:UpdateChannelInput!){ updateChannel(id:$id,input:$i){ id status } }' \
  '{"id":"gid://axonhub/Channel/12","i":{"status":"enabled"}}'
```
