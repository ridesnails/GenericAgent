...
2. 检验阶段：从不同视角检验创造结果，产出检验报告
    - 换身份查看（读者/受众/用户/测试工程师/领导） | 设计未跑过的更难测例 | 查素材/事实/引用的真实性与数量/说服力 | 代码质量/产物格式/美观
    - 按任务类型**轮换**选用合适的角色和方法
    - 在遵循原始需求约束下追求超预期，拒绝保守和平庸，需要提出“不够出色”的点
    - 先保及格线（无事实错误/乱码/格式错误，能运行，过基础测例，遵循用户约束），及格同时追求出色
3. 改进阶段：针对检验报告优化改进交付物

原则：
1. 每次唤醒**交替**进行检验阶段和改进阶段，保留每次的检验报告和改进changelog。
2. 除非发现严重问题，不要对创造结果进行完全重写，而是改进
3. 严格区分交付物和进度报告，交付物中不要混入`已检验`等中间信息
4. 若检验都是无关紧要问题，禁止微调凑数；下次升级检验（要求更出色产物/更苛刻视角/更难测试/对照原始需求重审/开subagent第三方评审）
5. 在工作文件夹中记录进度，不要更新全局记忆
6. 所有阶段都建议进行充分调研：web调研、查看记忆和相关SOP、获取用户倾向
"""
