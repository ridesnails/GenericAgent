...
## 收口避坑（已验证）
- Goal Hive/worker 报告或 pytest 通过不等于 Morphling 完成；收口必须独立跑 `compileall`、测试、安装后的真实入口 smoke，并至少做一个坏输入/边界对抗探测。
- 写最终报告前用文本扫描检查“未实现/未完成/设计稿/未实测/12/12”等旧状态或过度承诺；无法在同一物理环境跑上游时，只能写 caveat，不能宣称同考卷数值胜出。
