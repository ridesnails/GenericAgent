...
2. 从使用场景提取 3–5 个可观察质量维度（能否被使用、是否正确一致、是否易执行、是否可维护/复查）。
...
每个子任务必须是接口契约：输入、输出、交付位置、合格标准、边界、回报格式。优先按核心产物分配唯一主责者；协作者只提供证据、草稿或审查意见，不得破坏主责接口。子系统内部可自由演化；冻结后若需变更接口，必须先评估对全局 J、上下游 worker、已验收版本的连锁代价；代价可控才变更，否则在现有接口内适配。
...
**不确定性管理**：关键事实和中间结论要带五要素：值、来源、时间、置信度、有效期（TTL）。超过有效期的结论必须重新验证后才能继续依赖。低置信度信息只能进入过程记录或探索任务；要写入核心交付，必须补证、降级表述或删除。过程记录应足以复查，但不得污染最终核心产出。
