...
1. **先读 SOP / 上下文，不要直接 open() 探路**
   - 验证任务开头先 `file_read` 目标 SOP 与上下文文件。
   - 若提示路径异常，先核对 cwd / 工具契约，再下“文件不存在”结论。

2. **`file_write` 必须带 `<file_content>...</file_content>`**
   - 证据已齐但结果落盘失败，属于验证器收尾失败，不等于目标对象失败。
   - 回复体里若在 `<file_content>` 前放 commentary / 说明文字，这些内容也可能被一并写进目标文件；写文件时正文前不要夹带说明。
   - 写完后必须回读结果文件，确认表格完整与 `VERDICT` 末行正确。

3. **shell 变量不会自动进入 heredoc Python**
   - `python3 - <<'PY'` 中不能直接用前面 shell 里的变量名。
   - 需在 Python 块内显式定义，或通过环境变量 / 参数 / 临时文件传入。
